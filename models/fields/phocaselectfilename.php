...
class JFormFieldPhocaSelectFilename extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html = array();
		$link = 'index.php?option=com_phocafavicon&amp;view=phocafaviconi&amp;tmpl=component&amp;field='.$this->id;
		...
		$html[] = '		<a class="modal_'.$this->id.'" title="'.JText::_('COM_PHOCAFAVICON_FORM_SELECT_IMAGE').'"' .
							' href="'.($this->element['readonly'] ? '' : $link).'"' .
							' rel="{handler: \'iframe\', size: {x: 650, y: 375}}">';
		$html[] = '			'.JText::_('COM_PHOCAFAVICON_FORM_SELECT_IMAGE').'</a>';
		$html[] = '  </div>';
		$html[] = '</div>';*/
		...
		$html[] = '<input type="text" id="'.$this->id.'_id" name="'.$this->name.'" value="'. $this->value.'"' .
				' '.$attr.' />';
		$html[] = '<a class="modal_'.$this->id.' btn" title="'.JText::_('COM_PHOCAFAVICON_FORM_SELECT_IMAGE').'"'
				.' href="'.($this->element['readonly'] ? '' : $link).'"'
				.' rel="{handler: \'iframe\', size: {x: 650, y: 375}}">'
				. JText::_('COM_PHOCAFAVICON_FORM_SELECT_IMAGE').'</a>';
		$html[] = '</div>'. "\n";
